...
export class PromptBuilder {
    ...
    url(prUrl: string): PromptBuilder {
        return new PromptBuilder(prUrl, this._personas, this._mode);
    }

    personas(p: readonly Persona[]): PromptBuilder {
        return new PromptBuilder(this._url, p, this._mode);
    }

    mode(m: Modes): Prompt {
        return new PromptBuilder(this._url, this._personas, m).build();
    }
    ...
    build(): Prompt {
        ...
        const steps = this._personas.map((p, i) => new PersonaStepComponent(p, i + 1));
